...
namespace ableton {
    ...
    struct project {
        ...
        struct user_track : std::variant<audio_track, midi_track> {
            bool& envelope_mode_preferred();
            const bool& envelope_mode_preferred() const;
            track_delay& delay();
            const track_delay& delay() const;
            name& track_name();
            const name& track_name() const;
            std::optional<std::uint32_t>& color_index();
            const std::optional<std::uint32_t>& color_index() const;
            std::optional<std::uint32_t>& color();
            const std::optional<std::uint32_t>& color() const;
            std::int32_t& track_group_id();
            const std::int32_t& track_group_id() const;
            bool& track_unfolded();
            const bool& track_unfolded() const;
        };
